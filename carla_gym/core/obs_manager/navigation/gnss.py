...
class ObsManager(ObsManagerBase):
    ...
    def _define_obs_space(self):
        ...
        self.obs_space = spaces.Dict({
            ...
            'traj_points': spaces.Box(low=-1e3, high=1e3, shape=(15,), dtype=np.float32),
        })
    ...
    def get_observation(self):
        ...
        idx_inc = 0
        for _ in range(5):
            traj_gps_point, _ = global_plan_gps[self._idx + idx_inc]
            traj_points.append(np.array(traj_gps_point, dtype=np.float32))
            if self._idx + idx_inc < len(global_plan_gps) - 1:
                idx_inc += 1
        ...
        obs = {'gnss': gnss_data,
               ...
               'traj_points': np.concatenate(traj_points, axis=0)
               }
        return obs
