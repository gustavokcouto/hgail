...
multi_env_configs = [
    {"host": "192.168.0.6", "port": 2000, 'carla_map': 'Town02'},
]
...
def learn_bc(policy, device, expert_loader, eval_loader, env, resume_last_train):
    ...
    for i_episode in tqdm.tqdm(range(start_ep, episodes)):
        ...
        if i_steps - steps_last_eval > eval_step:
            ...
            avg_ep_stat, avg_route_completion, ep_events = WandbCallback.evaluate_policy_town2(env, policy, eval_video_path)
            env.reset()
...
if __name__ == '__main__':
    ...
    env = SubprocVecEnv([lambda env_id=env_id, config=config: env_eval_maker(env_id, config, obs_configs, env_wrapper_configs, rendering=True) for env_id, config in enumerate(multi_env_configs)])
    env.reset()
